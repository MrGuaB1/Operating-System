...
// get_pte - 获取pte并返回该pte的内核虚拟地址作为需要映射的线性地址
...
pte_t *get_pte(pde_t *pgdir, uintptr_t la, bool create) {
     ...
     *   KADDR(pa) : 获取物理地址并返回对应的内核虚拟地址
     *   set_page_ref(page,1) : 表示该页面被引用过一次
     *   page2pa(page): 获取该(struct Page *)页面管理的内存的物理地址
     *   struct Page * alloc_page() : 分配一个页
     *   memset(void *s, char c, size_t n) : 将 s 指向的内存区域的前 n 个字节设置为指定值 c
     *
     * DEFINEs:
     *   PTE_P           0x001                   // 页表/目录项
     * flags bit : Present
