...
/*  
 * sfs_io_nolock - Rd/Wr 从偏移位置到偏移长度磁盘块<-->缓冲区（在内存中）的文件内容
 * @sfs:      sfs file system
 * @sin:      sfs inode in memory
...
sfs_io_nolock(struct sfs_fs *sfs, struct sfs_inode *sin, void *buf, off_t offset, size_t *alenp, bool write) {
  ...
  // HINT: 调用sfs_bmap_load_nolock、sfs_rbuf、sfs_rblock等，读取文件中不同类型的块
	/*
	 * (1) 如果 offset 未与第一个块对齐，则 Rd/Wr 从 offset 到第一个块末尾的一些内容
	 *       NOTICE: useful function: sfs_bmap_load_nolock, sfs_buf_op
	 *               Rd/Wr size = (nblks != 0) ? (SFS_BLKSIZE - blkoff) : (endpos - offset)
	 * (2) Rd/Wr 对齐块 
	 *       NOTICE: useful function: sfs_bmap_load_nolock, sfs_block_op
     * (3) 如果结束位置与最后一个块不对齐，则读取/写入从开始到最后一个块的 (endpos % SFS_BLKSIZE) 的一些内容
	 *       NOTICE: useful function: sfs_bmap_load_nolock, sfs_buf_op	
	*/
    // 如果没对齐
