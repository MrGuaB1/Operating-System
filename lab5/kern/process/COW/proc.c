...
int do_exit(int error_code) {
    // 检查当前进程是否为 idleproc 或 initproc，如果是，则触发内核恐慌
    if (current == idleproc) {
        panic("idleproc exit.\n");
    }
    if (current == initproc) {
    ...
    // 将当前进程状态设置为僵尸状态（PROC_ZOMBIE）
    current->state = PROC_ZOMBIE;
    
    // 存储退出码到当前进程的 exit_code 属性中
    ...
    local_intr_save(intr_flag);
    {
        ...
        // 如果父进程等待子进程退出状态（WT_CHILD）
        if (proc->wait_state == WT_CHILD) {
            // 唤醒父进程
            wakeup_proc(proc);
        }

        // 将当前进程的子进程移交给 initproc 进程处理
    ...
    // 因为这个函数调用了 schedule，所以实际上不会返回到调用者
...
int do_wait(int pid, int *code_store) {
    ...
    // 检查传入的指针是否为空，如果不为空，则检查用户空间的内存访问权限
    if (code_store != NULL) {
        // 如果用户内存检查失败，则返回错误码 -E_INVAL
...
repeat:
    ...
    if (pid != 0) {
        // 查找进程号为 pid 的进程
        proc = find_proc(pid);
        // 如果找到了对应进程且该进程的父进程是当前进程
        if (proc != NULL && proc->parent == current) {
            haskid = 1;
            // 如果该进程处于僵尸状态，则跳转到找到进程的标签
            if (proc->state == PROC_ZOMBIE) {
                goto found;
            }
        }
    }
    // 如果 pid 为 0，即等待任一子进程结束
    else {
        // 从当前进程的子进程列表开始遍历
        proc = current->cptr;
        for (; proc != NULL; proc = proc->optr) {
            haskid = 1;
            // 如果子进程处于僵尸状态，则跳转到找到进程的标签
            if (proc->state == PROC_ZOMBIE) {
                goto found;
            }
        }
    ...
    // 如果存在子进程但都不是僵尸进程，则将当前进程状态设置为睡眠状态
    if (haskid) {
    ...
    // 如果不存在子进程，则返回错误码 -E_BAD_PROC
    return -E_BAD_PROC;

found:
    // 如果找到对应的进程是空闲进程或初始化进程，则触发内核恐慌
    if (proc == idleproc || proc == initproc) {
        panic("wait idleproc or initproc.\n");
    }
    // 如果 code_store 不为空，则将子进程的退出码存储到指定地址
    if (code_store != NULL) {
